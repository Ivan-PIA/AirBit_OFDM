...
std::vector<std::complex<double>> tx_rx(std::vector<std::complex<double>>  tx_data ){
    ...
    for (size_t step_mtu = 0; step_mtu < ((tx_data.size()/rx_mtu)+100); step_mtu++)
    {
        ...
        if (step_mtu <= tx_data.size()/rx_mtu-1){
            ...
            {
                tx_2[k2]   = (float)tx_data[((step_mtu) * rx_mtu) + i].real();
                tx_2[k2+1] = (float)tx_data[((step_mtu) * rx_mtu) + i].imag();
                k2 += 2;
            }
        }
        // fwrite(buffer, 2 * rx_mtu * sizeof(float), 1, file);   
        ...
        if (step_mtu <= ((tx_data.size()/rx_mtu)+100)-1){
            for(int i = 0; i < rx_mtu * 2; i += 2){
                ...
                // data.push_back(z);
                data2[((step_mtu) * rx_mtu) + k] = std::complex<double>(static_cast<float>(buffer[i]),static_cast<float>(buffer[i+1]));  // Добавлены скобки
